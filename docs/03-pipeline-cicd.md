...
recette (digest X)  ──imagetools create──▶  production (digest X)
...
- Créer les environnements `recette` et `production-simulee` (*Settings → Environments*).
- Aucune variable ni secret supplémentaire : `GITHUB_TOKEN` est fourni automatiquement.
